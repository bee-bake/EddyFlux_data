...
for(i in 1:length(n)) {
size = i
rm <- sample(all_48$co2_flux_umolm2s, size = i, replace = FALSE)
m_value <- mean(rm)
...
ggtitle("Distributions of half-hourly means randomly sampled from 48 data points
  (each line represents a complete set of means for all half hours)")
#theme(axis.title.x = element_blank())
#facet_wrap(~run_no, ncol = 2, scales = "free",
#labeller = labeller(run_no = c(`1` = "run-1",`2` = "run-2",`3` = "run-3",`4` = "run-4")) +
