...
#Updated: 21 May 2024 B Kandel - to include additional data for 2022-2024
...
eddy_flux <- read_csv("./Data/DataNotYetUploadedToEDI/EddyFlux_Processing/2024-05-10_EC_processed_withBDScript.csv") %>% 
  mutate(DateTime = as.POSIXct(DateTime, "%Y-%m-%d %H:%M:%S", tz = "EST")) %>% 
  filter(DateTime >= as.POSIXct("2020-04-05 20:00:00") & DateTime < as.POSIXct("2024-05-01 00:00:00"))
...
fcr_hourly <- eddy_flux %>% 
  ...
  mutate(DateTime = as.POSIXct(DateTime, format = "%Y-%m-%d %H", tz = "EST")) %>% 
  group_by(DateTime) %>% 
  ...
  summarise(NEE = mean(NEE_uStar_f, na.rm = TRUE),
            ...
            NEE50 = mean(NEE_U70_f, na.rm = TRUE),
            ...
            CH450 = mean(ch4_flux_U70_f, na.rm = TRUE),
...
fcr_monthly <- fcr_monthly[-c(13,29,30,31,32,33,34,35,36), ]
...
# Load in data
catwalk <- read.csv("./Data/FCR_Catwalk_2020May_2024April.csv") %>% 
  mutate(DateTime = as.POSIXct(DateTime, "%Y-%m-%d %H:%M:%S", tz = "EST")) %>% 
  filter(DateTime >= as.POSIXct("2020-04-05 20:00:00") & DateTime < as.POSIXct("2024-05-01 00:00:00"))
...
la <- read_csv("./Data/FCR_results_LA.csv") %>% 
  mutate(datetime = as.POSIXct(datetime, "%Y-%m-%d %H:%M:%S", tz = "EST")) %>% 
  filter(datetime >= as.POSIXct("2020-04-05") & datetime < as.POSIXct("2024-05-01")) %>% 
  rename(DateTime = datetime)

# Load in VT discharge: will need to be updated with final EDI & L1 data!
#inUrl1  <- "https://pasta.lternet.edu/package/data/eml/edi/202/11/aae7888d68753b276d1623680f81d5de" 
#infile1 <- paste0(getwd(),"/Data/VT_EDI_2015_2023.csv")
#download.file(inUrl1,infile1,method="curl")

q <- read_csv("./Data/VT_EDI_2015_2023.csv") %>% 
  mutate(DateTime = as.POSIXct(DateTime, "%Y-%m-%dT%H:%M:%SZ", tz = "EST")) %>% 
  filter(DateTime >= as.POSIXct("2020-04-05 01:00:00") & DateTime < as.POSIXct("2024-05-01 01:00:00"))
...
# Remove 2021 Apr when we don't have much data!!
env_monthly <- env_monthly[-c(13,29,30,31,32,33,34,35,36), ]
...
temp_time <- ggplot(env_daily,mapping=aes(x=DateTime,y=Temp_C_surface))+
  geom_vline(xintercept = as.POSIXct("2020-11-01"),linetype="dotted", color="black")+ #Turnover FCR; operationally defined
  ...
  geom_vline(xintercept = as.POSIXct("2021-11-03"), linetype = "dotted", color="black")+ #turnover
  geom_vline(xintercept = as.POSIXct("2022-01-11"), linetype = "dotted", color="blue")+
  geom_vline(xintercept = as.POSIXct("2022-02-10"), linetype = "dotted", color="red")+
  geom_vline(xintercept = as.POSIXct("2022-10-18"), linetype = "dotted", color="black")+ #turnover
  geom_vline(xintercept = as.POSIXct("2022-12-23"), linetype = "dotted", color="blue")+
  geom_vline(xintercept = as.POSIXct("2023-01-02"), linetype = "dotted", color="red")+
  geom_vline(xintercept = as.POSIXct("2023-01-15"), linetype = "dotted", color="blue")+
  geom_vline(xintercept = as.POSIXct("2023-02-05"), linetype = "dotted", color="red")+
  geom_vline(xintercept = as.POSIXct("2023-10-24"), linetype = "dotted", color="black")+
  geom_vline(xintercept = as.POSIXct("2024-01-08"), linetype = "dotted", color="blue")+
  geom_vline(xintercept = as.POSIXct("2024-01-26"), linetype = "dotted", color="red")+
...
all_cor <- bind_rows(hourly_cor,daily_cor,weekly_cor,monthly_cor)
...
best.vars <- colnames(co2_hourly_scale)[combn(cols,2)[,2]] # Change to reflect the best fit parameters as identified with 'best' #Num.covars-2, Covar.cols-2
best.vars.cols <- combn(cols,2)[,2] # Change to reflect the best fit parameters as identified with 'best' #Num.covars-2, Covar.cols-2
...
best.vars <- colnames(ch4_hourly_scale)[combn(cols,3)[,1]] # UPDATE THIS FOLLOWING 'BEST' #Num.covars-3, Covar.cols-1
best.vars.cols <- combn(cols,3)[,1] # UPDATE THIS FOLLOWING 'BEST' #Num.covars-3, Covar.cols-1
...
best.vars <- colnames(co2_daily_scale)[combn(cols,4)[,7]] # Change to follow 'best' #Num.covars-4, Covar.cols-7
best.vars.cols <- combn(cols,4)[,7] # Change to follow 'best' #Num.covars-4, Covar.cols-7
...
best.vars <- colnames(ch4_daily_scale)[combn(cols,1)[,1]] # Change this to reflect the 'best' #Num.covars-1, Covar.cols-1
best.vars.cols <- combn(cols,1)[,1] # Change this to reflect the 'best' #Num.covars-1, Covar.cols-1
...
best.vars <- colnames(co2_weekly_scale)[combn(cols,2)[,2]] # Change this to reflect 'best' #Num.covars-2, Covar.cols-2
best.vars.cols <- combn(cols,2)[,2] # Change this to reflect 'best' #Num.covars-2, Covar.cols-2
...
best.vars <- colnames(ch4_weekly_scale)[combn(cols,2)[,4]] # Change this to reflect 'best' #Num.covars-2, Covar.cols-4
best.vars.cols <- combn(cols,2)[,4] # Change this to reflect 'best' #Num.covars-2, Covar.cols-4
...
best.vars <- colnames(co2_monthly_scale)[combn(cols,2)[,1]] # Change this to reflect 'best' #Num.covars-2, Covar.cols-1
best.vars.cols <- combn(cols,2)[,1] # Change this to reflect 'best' #Num.covars-2, Covar.cols-1
...
best.vars <- colnames(ch4_monthly_scale)[combn(cols,2)[,3]] # Change this to reflect 'best' #Num.covars-2, Covar.cols-3
best.vars.cols <- combn(cols,2)[,3] # Change this to reflect 'best' #Num.covars-2, Covar.cols-3
