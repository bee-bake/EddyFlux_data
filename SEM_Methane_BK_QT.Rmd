...
<!--
Add this image to GitHub and make path relative to the project directory
-->

![](https://raw.github.com/bee-bake/EddyFlux_data/main/conceptual_diag/FCR_map_wEC.png)
...
Add this image to GitHub and make path relative to the project directory
-->
![](https://raw.github.com/bee-bake/EddyFlux_data/main/conceptual_diag/stream_team_schematic.png)
...
<!--
I am confused.  Is the flux data in the comp_data or in the ghg_FCR.  

Break this into multiple chunks so it is easier to see the intermediate results

How is https://raw.github.com/bee-bake/EddyFlux_data/main/comp_data.csv calculated?  
-->
...
# USE https://dplyr.tidyverse.org/reference/case_when.html
...
![](https://raw.github.com/bee-bake/EddyFlux_data/main/conceptual_diag/hyp_model.png)

![](https://raw.github.com/bee-bake/EddyFlux_data/main/conceptual_diag/epilimnetic_model.png)

![](https://raw.github.com/bee-bake/EddyFlux_data/main/conceptual_diag/flux_model_5m.png)

![](https://raw.github.com/bee-bake/EddyFlux_data/main/conceptual_diag/flux_model.png)
...
#fit the path models
model_flux <- psem(
  lm(sch_stab ~ temp_9m, data_flux),
  lm(DOsat_9m ~ sch_stab, data_flux),
...
#test of independent claims
m1 <- lm(iCH4_9m ~ DOsat_9m + sch_stab + temp_9m, data_flux)
summary(m1)

#re-fit the path models
model_flux <- psem(
  lm(sch_stab ~ temp_9m, data_flux),
  lm(DOsat_9m ~ sch_stab, data_flux),
  lm(iCH4_9m ~ DOsat_9m + temp_9m + sch_stab, data_flux)
...
model_flux <- '
  sch_stab ~ temp_9m
  DOsat_9m ~ sch_stab
  iCH4_9m ~ DOsat_9m + sch_stab + temp_9m
...
#fit the path models
model_flux <- psem(
  lm(DOsat_5m ~  sch_stab, data_flux),
  lm(iCH4_9m ~  DOsat_5m + sch_stab, data_flux),
  lm(iCH4_5m ~ iCH4_9m + DOsat_5m, data_flux)

)

summary(model_flux)

m1 <- lm(iCH4_9m ~ sch_stab + mean_fDOM + DOsat_5m, data_flux)
summary(m1)
...
model_flux <- '
  DOsat_5m ~  sch_stab + mean_fDOM
  iCH4_9m ~  DOsat_5m + sch_stab
  iCH4_5m ~ iCH4_9m + DOsat_5m + mean_fDOM
...
```{r}
#refit the path models
model_flux <- psem(
  lm(DOsat_1.6m ~  mean_Chla + iCH4_9m, data_flux),
  lm(iCH4_1.6m ~ DOsat_1.6m + mean_Chla + sch_stab + iCH4_9m + DOsat_5m, data_flux)
...
model_flux <- '
  DOsat_1.6m ~  mean_Chla + iCH4_9m 
  iCH4_1.6m ~ DOsat_1.6m + mean_Chla + sch_stab + iCH4_9m + DOsat_5m
...
model_flux <- '
  ...
  iCH4_5m ~ DOsat_5m + iCH4_9m + mean_fDOM
...
#Alternative model where we think that the fluxes via ebullition pathway from the bottom (9m) of the reservoir is more important than the one from metalimnetic (5m) pathway.
...
model_flux <- psem(
  ...
  lm(iCH4_9m ~ DOsat_9m + sch_stab, data_flux),
  lm(iCH4_5m ~ DOsat_5m + iCH4_9m + mean_fDOM, data_flux),
  lm(iCH4_1.6m ~ Wind + mean_Chla + sch_stab + iCH4_9m + iCH4_5m, data_flux),
...
model_flux <- '
  ...
  iCH4_9m ~ DOsat_9m + sch_stab
  iCH4_5m ~ DOsat_5m + iCH4_9m + mean_fDOM
  iCH4_1.6m ~ Wind + mean_Chla + sch_stab + iCH4_9m + iCH4_5m
...
```{r}
#reverse-diffusion and ebullition model
#fit the path models
model_flux <- psem(
  lm(sch_stab ~  temp_1.6m + Wind, data_flux),
  lm(iCH4_9m ~ DOsat_9m + sch_stab + iCH4_5m, data_flux),
  lm(iCH4_5m ~ DOsat_5m + iCH4_1.6m + mean_fDOM, data_flux),
  lm(iCH4_1.6m ~ Wind + DOsat_1.6m + mean_Chla + sch_stab, data_flux),
  lm(daily_CH4 ~ Wind + iCH4_1.6m + mean_Chla + temp_1.6m + iCH4_5m + iCH4_9m, data_flux)
...
model_flux <- '
  sch_stab ~ temp_1.6m + Wind
  iCH4_9m ~ DOsat_9m + sch_stab + iCH4_5m
  iCH4_5m ~ DOsat_5m + iCH4_1.6m + mean_fDOM
  iCH4_1.6m ~ Wind + DOsat_1.6m + mean_Chla + sch_stab
  daily_CH4 ~ Wind + iCH4_1.6m + mean_Chla + temp_1.6m + iCH4_5m + iCH4_9m
  iCH4_5m ~~  iCH4_1.6m
  DOsat_1.6m ~~ iCH4_9m
'
