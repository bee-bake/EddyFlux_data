...
met_all <- read_csv("https://pasta.lternet.edu/package/data/eml/edi/389/8/d4c74bbb3b86ea293e5c52136347fbb0",
                    col_select=c("DateTime","BP_Average_kPa","AirTemp_C_Average","RH_percent",
                                 "ShortwaveRadiationUp_Average_W_m2", "ShortwaveRadiationDown_Average_W_m2",
                                  ...
                                  "Albedo_Average_W_m2","WindSpeed_Average_m_s","WindDir_degrees", "Rain_Total_mm", "PAR_Total_mmol_m2"))%>%
...
met_30 <- met_all %>% 
  group_by(Breaks) %>% 
  select(-Rain_Total_mm,-PAR_Total_mmol_m2) %>%
...
#remove all the unmatched rows in one df to another
uneeded<-which(!rownames(ec2) %in% rownames(met2))    
ec2<-ec2[-uneeded,]
#plot the values
ggplot()+
...
#plot the data with paired matches only
plot(ec2$wind_speed_ms, met2$WS_ms_Avg[1:length(ec2$wind_speed_ms)])
...
uneeded<-which(!rownames(ec_filt) %in% rownames(met2))    
ec_filt<-ec_filt[-uneeded,]
...
uneeded<-which(!rownames(eddy_fcr) %in% rownames(met2))    
eddy_fcr<-eddy_fcr[-uneeded,]
ggplot()+
...
# Following Eddy Proc
eddy_fcr3 <- eddy_fcr3 %>%
   slice(-c(8968, 13537, 17448, 17968, 21550, 28825, 29085, 30193, 31504, 35180, 37317, 41564))
